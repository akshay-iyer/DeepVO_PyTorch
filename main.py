...
# a - append to file; w - write to file; both of them create file if not exists
mode = 'a' if par.resume else 'w'
print ("mode= ",mode)
print("record path: ", par.record_path)
...
if torch.cuda.device_count() > 1:
  ...
  # dim = 0 [30, xxx] -> [10, ...], [10, ...], [10, ...] on 3 GPUs
    M_deepvo = nn.DataParallel(M_deepvo)
...
if par.resume:
	M_deepvo.load_state_dict(torch.load(par.load_model_path), strict = False)
...
for ep in range(par.epochs):
	...
	print('='*50)
	# Train
	# model.train() function doesn't actually train but it just is like an indication to pytorch thatthe following is for training and parameters need to be saved
	M_deepvo.train()
	
	loss_mean = 0
	t_loss_list = []
	#print ("train dl is: ",train_dl)
	# train_dl is the entire trg dataset
	for _, t_x, t_y in train_dl:
		# t_x, t_y are the corresponding input and output pair, the first _ might be the index
		if use_cuda:	
			t_x = t_x.cuda(non_blocking=par.pin_mem)
			t_y = t_y.cuda(non_blocking=par.pin_mem)
		
		ls = M_deepvo.module.step(t_x, t_y, optimizer).data.cpu().numpy()
...
# till here, we have gone thru entire trg and validn dataset once. thus 1 epoch is complete
	...
	if loss_mean_valid < min_loss_v and ep % check_interval == 0:
		...
		f.write('Save model at ep {}, mean of valid loss: {}'.format(ep+1, loss_mean_valid))
	...
	if loss_mean < min_loss_t and ep % check_interval == 0:
		...
		f.write('Save model at ep {}, mean of train loss: {}'.format(ep+1, loss_mean))
